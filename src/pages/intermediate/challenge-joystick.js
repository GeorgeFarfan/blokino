...
let validate_result_code = [[], [], []],
    ...
    validator = require("../../utils/validators/joystick-validator");

$("#newVariableModal").on("hidden.bs.modal", () => {
    let new_variable = $("#new-variable")
        .val()
        .replace(/\s/g, "");
    if (new_variable === "LED" || new_variable === "JOYSTICK") {
    ...
    if (device !== undefined) {
        if (current_test == "test_1") {
            ...
            if (validate_code !== "Error") {
                validate_result_code = validator.joystick(validate_code.body, validate_result_code);
                let validate_result = validate_result_code.filter(
                    potentiometer_filter =>
                        potentiometer_filter["name"] === "LED" ||
                        potentiometer_filter["name"] === "JOYSTICK"
                );
                            ...
                            let result_filter = data.variables_status.filter(
                                led_filter =>
                                    (led_filter["custom"]["type"] === "LED" &&
                                        led_filter["pin"] === 13) ||
                                    (led_filter["custom"]["type"] === "JOYSTICK" &&
                                        led_filter["custom"]["pos_x"] === "A0" &&
                                        led_filter["custom"]["pos_y"] === "A1")
                            );
