 ...
 * @description Este archivo contiene toda la configuracion, que necesita el desafío del MOTOR.
 */
...
let validate_result_code = [[], [], []],
    ...
    validator = require("../../utils/validators/motor-validator");

$("#newVariableModal").on("hidden.bs.modal", () => {
    let new_variable = $("#new-variable")
        .val()
        .replace(/\s/g, "");
    if (new_variable === "MOTOR" || new_variable === "TECLADO") {
    ...
    if (device !== undefined) {
        if (current_test == "test_1") {
            ...
            if (validate_code !== "Error") {
                validate_result_code = validator.motor(validate_code.body, validate_result_code);
                let validate_result = validate_result_code.filter(
                    potentiometer_filter =>
                        potentiometer_filter["name"] === "TECLADO" ||
                        potentiometer_filter["name"] === "MOTOR"
                );
                    ...
                    ipcRenderer.on("channel_one", (event, child_result) => {
                        if (
                            ...
                            let result_filter = data.variables_status.filter(
                                led_filter =>
                                    (led_filter["opts"] &&
                                        led_filter["opts"]["custom"]["type"] === "MOTORS" &&
                                        led_filter["pin"] === 5) ||
