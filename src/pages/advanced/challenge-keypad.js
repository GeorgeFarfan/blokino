...
$("#newVariableModal").on("hidden.bs.modal", () => {
    let new_variable = $("#new-variable")
        ...
        .replace(/\s/g, "");
    if (
        new_variable === "TECLADO" ||
        new_variable === "SERVOMOTOR" ||
        new_variable === "LED" ||
        new_variable === "ZUMBADOR"
    ) {
    ...
    if (device !== undefined) {
        if (current_test == "test_1") {
            ...
            if (validate_code !== "Error") {
                validate_result_code = validator.keypad(validate_code.body, validate_result_code);
                let validate_result = validate_result_code.filter(
                    potentiometer_filter =>
                        potentiometer_filter["name"] === "LED" ||
                        potentiometer_filter["name"] === "TECLADO" ||
                        potentiometer_filter["name"] === "SERVOMOTOR" ||
                        potentiometer_filter["name"] === "ZUMBADOR"
                );
                if (validate_result.length == 4) {
                    ...
                    ipcRenderer.send("execute", {
                        ...
                        device: device,
                        validate_code: {
                            variable: [`LED`, `SERVOMOTOR`, `TECLADO`, `ZUMBADOR`]
                        },
                        channel: "channel_one"
                    });
                    ipcRenderer.on("channel_one", (event, child_result) => {
                        if (
                            ...
                            modalChallenges(child_result, current_test, 1, false);
                        } else {
                            ...
                            let result_filter = data.variables_status.filter(
                                led_filter =>
                                    (led_filter["custom"]["type"] === "LED" &&
                                        led_filter["pin"] === 8) ||
                                    (led_filter["custom"]["type"] === "SERVOMOTOR" &&
                                        led_filter["pin"] === 4) ||
                                    led_filter["custom"]["type"] === "KEYPAD" ||
                                    (led_filter["custom"]["type"] === "BUZZER" &&
                                        led_filter["pin"] === 7)
                            );
                            if (result_filter.length == 4) {
                                modalChallenges(data.status, current_test, 1, true);
                            } else {
                                modalChallenges("Error", current_test, 1, false);
                            }
                        }
                    });
                } else {
