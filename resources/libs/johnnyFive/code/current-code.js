'use strict';const jc = require('json-cycle'),chalk = require('chalk'),log = console.log;let five = require('johnny-five');try {let board = new five.Board({ port:'COM4', repl: false, debug: false});board.on('ready', () => { try {let LCD;
LCD = (new five.LCD({
    controller: 'PCF8574',
    custom: {
        type: 'LCD',
...
LCD.useChar("bell");
LCD.useChar("check");
LCD.cursor(0, 0).print(([':note:', ':check:', 'P'].toString().replace(/,/g, " ")).substr(0, 20));
LCD.cursor(1, 0).print(':smile: :smile::smile:'.substr(0, 20));
LCD.cursor(2, 0).print(':bell:'.substr(0, 20));
LCD.cursor(3, 0).print(':check:'.substr(0, 20));
