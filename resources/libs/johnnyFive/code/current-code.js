'use strict';const jc = require('json-cycle'),chalk = require('chalk'),log = console.log;let five = require('johnny-five');try {let board = new five.Board({ port:'/dev/ttyACM1', repl: false, debug: false});board.on('ready', function () { try {var led_1, led_2, led_3;function ParpadearLEDs() {led_1.blink(1000);led_1.custom.blink=1000;led_2.blink(100);led_2.custom.blink=100;led_3.blink(300);led_3.custom.blink=300;}led_1 = (new five.Led({pin:10,custom:{type:'LED',blink:0}}));led_2 = (new five.Led({pin:11,custom:{type:'LED',blink:0}}));led_3 = (new five.Led({pin:12,custom:{type:'LED',blink:0}}));ParpadearLEDs();
            function getDataVariable(){
                let tmp=[];
                [led_1,led_2,led_3].forEach(function(ele) {
